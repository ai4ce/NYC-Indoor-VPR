...
def test(args, eval_ds, model, test_method="hard_resize", pca=None):
    ...
    all_features=anyloc_vlad_generate.all_desc()
    ...
    # args.recall_values by default is [1, 5, 10, 20]
    recalls = np.zeros(len(args.recall_values))
    # print(predictions.shape,positives_per_query.shape)
    # f=open("anyloc.txt","w")
    for query_index, pred in enumerate(predictions):
        # bl=[False,False,False,False]
        for i, n in enumerate(args.recall_values):
            if np.any(np.in1d(pred[:n], positives_per_query[query_index])):
                recalls[i:] += 1
                ...
                #     bl[j]=True
                break
        # f.write(str(query_index)+' '+str(pred[0])+'\n')
